...
def loopback_type_select():
    ...
    print('    USB/UART: 1. To test port speed')
    print('    Simple: 2. To check if the port is working')
...
def simple_loopback(com_port, com_speed, packet_number=1024, packet_size=1):
    checked_packs = 0
    corrupted_packs = 0

    try:
        with serial.Serial(com_port, com_speed, timeout=1) as com_port:
            print("\nPacks statistic:")
...
def speed_loopback(com_port, usb_port, speed, packet_number=1024, packet_size=1024):
    checked_packs = 0
    corrupted_packs = 0

    try:
        with serial.Serial(com_port, speed, timeout=1) as com_port, \
             serial.Serial(usb_port, speed, timeout=1) as usb_port:
